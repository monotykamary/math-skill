...
&& \text{(quotient rule)}, \\
u_x &= -2 \nu \frac{\varphi_{xx} \varphi - \varphi_x^2}{\varphi^2}
&& \text{(quotient rule)}, \\
u_{xx} &= -2 \nu \Bigl(\frac{\varphi_{xxx}}{\varphi}
- 3 \frac{\varphi_{xx} \varphi_x}{\varphi^2} + 2 \frac{\varphi_x^3}{\varphi^3}\Bigr)
\end{aligned}
$$

(the second quotient: the derivative of $\varphi_{xx}/\varphi$ contributes
$\varphi_{xxx}/\varphi - \varphi_{xx} \varphi_x/\varphi^2$ and the
derivative of $\varphi_x^2/\varphi^2$ contributes
$2 \varphi_x \varphi_{xx}/\varphi^2 - 2 \varphi_x^3/\varphi^3$, summed).
Pair the flux term with the Laplacian term before summing:

$$
\begin{aligned}
u u_x - \nu u_{xx}
&= 4 \nu^2 \frac{\varphi_x \varphi_{xx}}{\varphi^2}
- 4 \nu^2 \frac{\varphi_x^3}{\varphi^3}
+ 2 \nu^2 \frac{\varphi_{xxx}}{\varphi}
- 6 \nu^2 \frac{\varphi_{xx} \varphi_x}{\varphi^2}
+ 4 \nu^2 \frac{\varphi_x^3}{\varphi^3} \\
&= 2 \nu^2 \frac{\varphi_{xxx}}{\varphi}
- 2 \nu^2 \frac{\varphi_{xx} \varphi_x}{\varphi^2}
\end{aligned}
...
$$
u_t + u u_x - \nu u_{xx} = -2 \nu \frac{\varphi_{xt}}{\varphi}
+ 2 \nu \frac{\varphi_t \varphi_x}{\varphi^2}
+ 2 \nu^2 \frac{\varphi_{xxx}}{\varphi}
- 2 \nu^2 \frac{\varphi_{xx} \varphi_x}{\varphi^2},
$$

which is exactly $-2 \nu$ times the $x$-derivative of
$(\varphi_t - \nu \varphi_{xx})/\varphi$ (the quotient rule on that
...
$u$ solves forces $(\varphi_t - \nu \varphi_{xx})/\varphi = f(t)$ for a
...
$\varphi(0,x) = \exp(-\Phi_0(x)/(2 \nu))$ with $\Phi_{0,x} = u_0$
(integrate $u_0$ once; the additive constant is gauge and cancels in
$u$). The heat kernel of case-heat-kernel.md section 2 and
case-black-scholes.md section 5 then gives

$$
\varphi(t,x) = \text{constant} \cdot \int \exp\Bigl(-\frac{\Phi_0(y) + (x-y)^2/(2t)}{2 \nu}\Bigr) \, dy,
$$
...
$$
\min_y \bigl[\Phi_0(y) + (x-y)^2/(2t)\bigr]
$$
...
$$
u(t,x) = \frac{d}{dx} \min_y \bigl[\Phi_0(y) + (x-y)^2/(2t)\bigr].
$$
...
$u_t = -x/(1+t)^2$, $u_x = 1/(1+t)$, $u_{xx} = 0$, $u u_x = x/(1+t)^2$, so
$u_t + u u_x = 0 = \nu u_{xx}$: the explicit flow satisfies the viscous
...
$u_0(x) = -x$: characteristics $dx/dt = u$ start at slope $-1$ and
...
$u_l = 1 > 0 > -1 = u_r$: admissible. PROVEN for this datum.
...
constant in time on each side, so the $\varphi_t$ terms contribute
nothing; the $\varphi_x$ boundary terms cancel exactly through
$1/2 - 1/2$).

## 7. The viscous profile is a tanh, and its entropy bill is exact

Seek a stationary viscous wave solving $0 = -u u_x + \nu u_{xx}$, so
$\nu u_{xx} = u u_x$. Treat this as $(\nu u_x - u^2/2)_x = 0$ (the chain
rule on $u^2/2 = u u_x$, named). Integrate: $\nu u_x = u^2/2 + C$.
Boundary conditions $u(\pm\infty) = \mp 1$ settle $C = -1/2$ (at either
end $u_x \to 0$ and $u^2/2 \to 1/2$; the constant is the same, signed
bookkeeping stated).

$\nu u_x = (u^2 - 1)/2$. Separate and integrate:
...
$d\,\mathrm{sech}^2/d\xi = -2\, \mathrm{sech}^2 \tanh$, the second
derivative gains one more factor $1/(2 \nu)$, in total
$+(1/(2 \nu^2)) \mathrm{sech}^2 \xi \tanh \xi$; the sign passes the
check below). Substitute into
$\nu u_{xx} - u u_x = \mathrm{sech}^2 \tanh/(2 \nu)
- (-\tanh)(-(1/(2 \nu)) \mathrm{sech}^2)
= \mathrm{sech}^2 \tanh/(2 \nu)
- \tanh\, \mathrm{sech}^2/(2 \nu) = 0$. PROVEN: the tanh is an
exact stationary viscous shock of width proportional to $\nu$, sitting
between $u = 1$ at $-\infty$ and $u = -1$ at $+\infty$. Letting
$\nu \to 0$ recovers the Rankine-Hugoniot jump of section 6: the viscous
repair of the shock is a profile, not a patch.

The entropy bill: for the entropy-entropy flux pair $(u^2/2, u^3/3)$
(the flux of the entropy is $\int \eta'(u) f'(u) \, du$, a one-line
chain computation), the viscous production per unit time is
$\nu \int (u_x)^2 \, dx$ (the classical strong-form computation: dotted
terms shown in the next sentence; the derivative of $u^2/2$ along the
viscous flow equals $-u u u_x + \nu u u_{xx}
= -(u^3/3)_x + \nu (u u_x)_x - \nu (u_x)^2$, every equality a product
rule, hence the divergence form plus the negative defect
$-\nu u_x^2$). Compute the integral for the tanh wave exactly:

$$
\nu \int (u_x)^2 \, dx = \frac{1}{4 \nu} \int \mathrm{sech}^4\Bigl(\frac{x}{2 \nu}\Bigr) \, dx
= \frac{1}{2} \int \mathrm{sech}^4 \xi \, d\xi
$$

($\xi := x/(2 \nu)$, $dx = 2 \nu \, d\xi$), and the primitive of
$\mathrm{sech}^4$ is $\tanh - \tanh^3/3$ (check by
differentiation: $\mathrm{sech}^2 - \tanh^2 \mathrm{sech}^2
= \mathrm{sech}^2(1 - \tanh^2) = \mathrm{sech}^4$, an identity of
hyperbolic functions named per line), so the integral equals
...
$\nu = 1/50$. Wave halfway points: $u(0.01) = -\tanh(0.01 \cdot 25)
= -\tanh(0.25) = -0.2449$ ($\tanh$ by the $(e^{0.5}-1)/(e^{0.5}+1)$ form,
table arithmetic), $u(0.1) = -\tanh(2.5) = -0.9866$: essentially the full
jump within $|x|$ of order $2 \nu = 0.04$. As $\nu$ shrinks the same function
squeezes toward the jump; the entropy bill stays 2/3 at every $\nu$
(section 7's integral does not depend on $\nu$ at all). SUPPORTED
arithmetic, and the $\nu$-independence is PROVEN.
