...
$f(w) = \ln w$ has $f'(w) = 1/w$ and $f''(w) = -1/w^2$ (derivatives of
$\ln$, the standard table entry the course committed in case-kelly.md
section 3). Ito's formula for the scalar process with the quadratic
...
fluctuation scale of the discrete multiplicative game (case-kelly.md
sections 2 and 5), the same scale that produced Brownian motion in
case-black-scholes.md section 1.
...
the optimizer) is cited machinery, and for log utility the Itô-integral
direct argument of section 3 certifies the candidate independently (the
course's two instruments agree).
...
- Robust control: if the drift $\mu$ is not known, the maximization over
  $\pi$ turns into a sup-inf game (Hansen-Sargent), the continuous
  sibling of the distributionally robust Kelly of case-kelly.md section 7.
- Rough volatility breaks the bracket computation itself: with
  $H \sim 0.1$ the quadratic variation statement $(dB)^2 = dt$ has a
  fractional twin and the HJB of section 4 needs the rewriting flagged in
