...
Scores $s_1, s_2, \dots$ are i.i.d., mean 0, variance $\sigma^2$, mgf
$m(t) = \mathbb{E} e^{t s}$. The heat $W_n$ updates as in case-ewma.md:
$W_{n+1} = (1-\rho) s_{n+1} + \rho W_n$. The target event:
$W_n \ge \theta$ for some threshold. Question one: the rate at which the
stationary heat sits above $\theta$. Question two: how close the
finite-time heat is to stationary before we may use that rate.

Mean 0 keeps $W_n$ mean 0 at every $n$; the threshold asks for an
...
latest score reads $s_0$ and $W^* := (1-\rho) \sum_{j \ge 0} \rho^j s_{-j}$:
...
The one-step shift is $W^*_1 := (1-\rho) \sum_{j \ge 0} \rho^j s_{1-j}$.
Covariance:

$$
\mathrm{Cov}(W^*, W^*_1) = (1-\rho)^2 \sum_{j \ge 1} \rho^j \rho^{j-1} \sigma^2
$$
...
$$
\begin{aligned}
\mathrm{Cov} &= (1-\rho)^2 \sigma^2 \rho^{-1} \sum_{j \ge 1} \rho^{2j}
= (1-\rho)^2 \sigma^2 \rho \sum_{j \ge 0} \rho^{2j} \\
&= (1-\rho)^2 \sigma^2 \rho / (1-\rho^2)
= \sigma^2 \rho (1-\rho)/(1+\rho).
\end{aligned}
$$

The geometric identity $\sum_{j \ge 0} \rho^j = 1/(1-\rho)$ proved in
...
Claim 2: $\mathrm{Var}(W^*) = \sigma^2 (1-\rho)/(1+\rho)$. Independence
makes the variance additive across $j$ (covariances across distinct $j$
vanish, justified exactly as in case-black-scholes.md section 1), giving
$(1-\rho)^2 \sigma^2 \sum_{j \ge 0} \rho^{2j}
= (1-\rho)^2 \sigma^2/(1-\rho^2)
= \sigma^2 (1-\rho)/(1+\rho)$, dividing numerator and denominator by
$(1-\rho)$ (nonzero since $\rho < 1$) where
$(1-\rho^2) = (1-\rho)(1+\rho)$. PROVEN.

## 3. The exact log-mgf from self-similarity

$W^* = (1-\rho) s_0 + \rho$ (the same series from $j = 1$ scaled), and
that remainder is independent of $s_0$ (the sets of indices $\{0\}$ and
$\{1,2,\dots\}$ are disjoint, and independence of the i.i.d. sequence is
the named premise) with the same law as $W^*$ (stationarity of the
backward shift). Hence $M(t) := \mathbb{E} e^{tW^*}$ satisfies
$M(t) = \mathbb{E} e^{t(1-\rho)s_0} \, \mathbb{E} e^{t \rho W^*}
...
$$
M(t) = \Bigl[\prod_{j = 0}^{J-1} m((1-\rho) \rho^j t)\Bigr] M(\rho^J t).
$$
...
$$
\log M(t) = \sum_{j \ge 0} \log m((1-\rho) \rho^j t) =: \Lambda(t).
$$
...
$$
\begin{aligned}
\Lambda(t) &= (\sigma^2 t^2/2)(1-\rho)^2 \sum_{j \ge 0} \rho^{2j}
...
The Chernoff rate: $I(\theta) = \sup_{t \ge 0} [t \theta - \Lambda(t)]$.
...
$W^* - W_n = (1-\rho) \sum_{j \ge n} \rho^j s_{\dots}$. Variance gap:

$$
\mathrm{Var}(W^* - W_n) = \sigma^2 (1-\rho)^2 \sum_{j \ge n} \rho^{2j}
...
Poisson process with rate $\lambda$. $Y_i$ are i.i.d. $\mathrm{Exp}(\gamma)$:
density $\gamma e^{-\gamma y}$, mean $1/\gamma$, mgf
$\mathbb{E} e^{rY} = \gamma/(\gamma-r)$ for $r < \gamma$ (computed by
direct integration:
$\int_0^\infty \gamma e^{-\gamma y} e^{r y} \, dy
= \gamma \int e^{-(\gamma-r)y} \, dy = \gamma/(\gamma-r)$; each step
named). Premium income flows at rate $c$. Surplus:
$X_t = u + c t - \sum_{i \le N_t} Y_i$, where $N_t$ is the Poisson count.
Ruin time $T = \inf\{t : X_t < 0\}$; ruin probability
...
$$
\mathbb{E}[M_{t+u} \mid \mathcal{F}_t]
= M_t \mathbb{E}[e^{-r^*(c u - \sum_{\text{fresh}} Y_i)}]
= M_t e^{-r^* c u} e^{\lambda u (\mathbb{E} e^{rY} - 1)},
$$

where the compound Poisson exponential identity is PROVEN by conditioning
on the number of fresh claims: $\sum_{k \ge 0} \mathbb{P}(N = k)
(\mathbb{E} e^{rY})^k$ with Poisson weights
$e^{-\lambda u}(\lambda u)^k/k!$ sums to
$e^{\lambda u(\mathbb{E} e^{r Y}-1)}$, the Taylor series of the
exponential recognized term by term. The adjustment equation makes the
bracket $\lambda(\mathbb{E} e^{r^*Y} - 1) - c r^* = 0$, so
$\mathbb{E}[M_{t+u} \mid \mathcal{F}_t] = M_t$: a martingale. The
optional stopping at the ruin time belongs to the martingale machinery
of the course: a nonnegative process with a conditional-mean-1 increment
satisfies $\mathbb{E}[M_{T \wedge t}] \le M_0$ (optional sampling for
nonnegative supermartingales, cited; note $\mathbb{E} M_{T \wedge t}
\le \mathbb{E} M_0$ in fact holds with the supermartingale sign and
equality requires care that ruinicity alone does not give). Tag GAP: the
stopping-time justification.

On the set $\{T \le t\}$, $X_T < 0$ (ruin means strictly negative at the
first touch; the surplus has downward jumps, so the first touch passes
below 0), hence $-r^* X_T > 0$ and $M_T = e^{-r^* X_T} \ge 1$. Therefore
$\mathbf{1}_{\{T \le t\}} \le M_{T \wedge t}$ pointwise (on the ruin
event $M_T \ge 1$ given the indicator is 1; off the event the indicator
is $0 \le$ the nonnegative $M$). Taking expectations and passing
$t \to \infty$ (monotone convergence on the increasing events, named):

$$
\psi(u) = \mathbb{P}(T < \infty) = \lim_t \mathbb{P}(T \le t)
\le \lim_t \mathbb{E} M_{T \wedge t} \le M_0 = e^{-r^* u}.
$$
