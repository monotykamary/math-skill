...
$N$ states, $i = 1..N$, energy $E_i$ on state $i$. A probability
distribution $p$ on the states. Entropy (in nats):
$H(p) = -\sum_i p_i \ln p_i$, with $0 \ln 0$ set to $0$ (the extension
...
$p_i = e^{\beta(\psi - E_i)}$ with some scalar
$\beta$ and normalizer.
...
$x = q_i/p_i$ (each ratio is positive when $p_i > 0$ on the support,
checked termwise):

$$
\sum_i q_i \ln(q_i/p_i) \ge \sum_i q_i (1 - p_i/q_i)
= \sum_i q_i - \sum_i p_i = 1 - 1 = 0.
$$

Each step: multiply the tangent inequality by $q_i \ge 0$ (sign
...
$D(q \,\|\, p) \ge 0$, equality exactly at $q = p$. This one inequality
runs the whole case.

## 3. Gibbs is the unique entropy maximizer

Fix $\beta$ for now, and define the candidate
$p_i = e^{-\beta E_i}/Z$ with $Z(\beta) := \sum_i e^{-\beta E_i}$.
For ANY $q$ on the simplex:

$$
-\sum_i q_i \ln p_i = \sum_i q_i (\beta E_i + \ln Z)
= \beta\Bigl(\sum_i q_i E_i\Bigr) + \ln Z.
$$

(Each step: substitute $\ln p_i = -\beta E_i - \ln Z$; linearity of the
sum; $\sum_i q_i = 1$.) If $q$ also meets the mean constraint,
$\sum_i q_i E_i = U$, so $-\sum_i q_i \ln p_i = \beta U + \ln Z$, the same
number for every admissible $q$. Apply section 2 with reference $p$:
$D(q \,\|\, p) = -H(q) + \sum_i q_i(-\ln p_i)$? Check the exact identity:
$D(q \,\|\, p) = \sum_i q_i \ln q_i - \sum_i q_i \ln p_i
= -H(q) - \sum_i q_i \ln p_i$. Therefore

$$
H(q) = -D(q \,\|\, p) - \sum_i q_i \ln p_i
\le -\sum_i q_i \ln p_i = \beta U + \ln Z.
$$
...
$p_i = e^{-\beta E_i}/Z(\beta)$ maximizes $H$ uniquely under mean
...
The relation between $\beta$ and $U$ is monotone. On the two-state example
in section 5, this is visible; the general fact is a derivative
computation identical to the convexity proof of
case-large-deviations.md section 3: substitute $t = -\beta$ there (their
name $Z_t$, tilt toward low energy, is our $e^{\beta(\dots)}$ up to sign).
$\frac{dU}{d\beta} = \sum_i E_i \frac{d p_i}{d\beta}$ and
$\frac{d}{d\beta} \frac{e^{-\beta E_i}}{Z}
= p_i\bigl(-E_i + \frac{d(\ln Z)}{d\beta}\bigr)$ by the chain rule split
of the log: $\frac{d}{d\beta} \ln Z
= \frac{1}{Z} \sum_i (-E_i) e^{-\beta E_i} = -U$ (derivative of the log
of a sum, applied at each term). Then
$\frac{dU}{d\beta} = \sum_i E_i p_i (-E_i + U)
= -(\mathbb{E}[E^2] - U^2) = -\mathrm{Var}_p(E) \le 0$: the derivative is
a variance, exactly the tilted-variance identity of
...
decreases, strictly when energies differ. So the matching $\beta(U)$
exists and is unique on the attainable range of $U$ (monotone functions
have at most one inverse value by definition; existence by continuity of
$U$ in $\beta$: $p$ is continuous in $\beta$ being a quotient of
continuous sums, and the limits $\beta \to \pm\infty$ reach the extreme
energies $\min E_i$ and $\max E_i$; tagged SUPPORTED with the limits
computed explicitly in section 5; the intermediate value theorem is the
cited calculus premise).

## 5. The tilt is exactly the Gibbs distribution: rate = entropy deficit

Put the uniform measure as the reference, $p_0 = (1/N, \dots, 1/N)$, the
...
parameter $t$: $q_t(i) = p_0(i) e^{tE_i}/Z(t)$,
$Z(t) = \sum_i p_0(i) e^{tE_i}$. At $t = -\beta$, $q_t = p_\beta$, the
Gibbs state with $Z(-\beta) = Z(\beta)/N$ (because $Z(t)$ is the
$(1/N)$-normalized partition: $Z(t) = (1/N) \sum e^{tE_i}$ and
$\sum e^{-\beta E_i} = N Z(-\beta)$; section 2's normalization just
divides by $N$, stated).

Two evaluations of the same number. First, the relative entropy route:
$D(p_\beta \,\|\, p_0) = \sum_i p_i \ln(p_i/(1/N)) = \ln N - H(p_\beta)$.
Each term: $\ln(p_i/(1/N)) = \ln p_i + \ln N$; the first part gives $-H$,
the second gives $\ln N$ times $\sum p = \ln N$. Second, the tilt
identity: for $q_t$ and $E_t := \mathbb{E}_{q_t}[E]$:
...
At the matching $t^*$ with $E_{t^*} = U$ (section 4's uniqueness), this
...
evaluations: $I(U) = \ln N - H(p_\beta)$, where $\beta$ matches $U$. Both
...
$E \in \{-h, +h\}$. $Z(\beta) = e^{\beta h} + e^{-\beta h}$ (sum over
the two states) $= 2 \cosh(\beta h)$ (definition of
$\cosh$: $(e^a + e^{-a})/2$, doubled). Mean energy:

$$
U = \sum_i p_i E_i = (+h)\frac{e^{-\beta h}}{Z} + (-h)\frac{e^{\beta h}}{Z}
...
baseline $p_0 = (1/2,1/2)$; $N = 2$, so $\ln Z(t) = \ln \cosh t$). The
